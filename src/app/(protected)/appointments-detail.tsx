import { AntDesign, FontAwesome, MaterialIcons, Ionicons, Feather, MaterialCommunityIcons, Entypo } from "@expo/vector-icons";
import { useLocalSearchParams, useRouter } from "expo-router";
import React, { useState, useEffect } from "react";
import { View, Text, TouchableOpacity, Alert, ScrollView, ActivityIndicator, Image, Dimensions } from "react-native";
...
const DetailItem = ({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) => (
  <View className="mb-4">
    <View className="flex-row items-center mb-1">
      ...
      <Text className="text-[#8B949E] text-xs font-medium">{label}</Text>
    </View>
  ...
  if (loading || !appointment) {
    return (
      <View className="flex-1 bg-[#0D1117] items-center justify-center">
        ...
        <Text className="text-[#00AEEF] mt-4 font-medium">Đang tải thông tin lịch hẹn...</Text>
      </View>
    );
  }
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            ...
            {status === "CONFIRMED" && (
              ...
              >
                <View className="bg-white/20 p-2 rounded-full">
                  <MaterialCommunityIcons name="ticket-confirmation" size={22} color="white" />
                </View>
                <View className="ml-3">
        ...
        <View className="p-5">
          ...
          >
            <View className="p-4 border-b border-[#30363D]">
              <View className="flex-row justify-between items-center">
                <View className="flex-row items-center">
                  <MaterialCommunityIcons name="calendar-clock" size={20} color="#00AEEF" />
            ...
            <View className="p-4">
              <View className="flex-row items-center mb-3">
                <View className="bg-[#21262D] rounded-lg p-3 mr-4">
                  <MaterialIcons name="event" size={30} color="#00AEEF" />
                </View>
                <View>
                  <Text className="text-white text-lg font-bold">{formattedDate}</Text>
                  <Text className="text-[#8B949E]">{formattedTime}</Text>
                </View>
              </View>
              
              <View className="mt-2 bg-[#21262D]/50 rounded-lg p-3 flex-row items-center">
                <MaterialIcons name="info-outline" size={20} color="#00AEEF" />
                <Text className="text-[#8B949E] ml-2 flex-1">
                  {isUpcoming 
          ...
          >
            <View className="p-4 border-b border-[#30363D]">
              <View className="flex-row items-center">
                <MaterialCommunityIcons name="card-account-details" size={20} color="#00AEEF" />
            ...
            <View className="p-4">
              <DetailItem 
                icon={<FontAwesome name="user-md" size={22} color="#00AEEF" />}
                label="BÁC SĨ"
                value={doctor?.fullName || "Chưa xác định"}
              />
              
              <DetailItem 
                icon={<MaterialIcons name="location-on" size={22} color="#00AEEF" />}
                label="ĐỊA ĐIỂM"
                value="Phòng khám Medical Center"
              />
              
              <DetailItem 
                icon={<MaterialIcons name="confirmation-number" size={22} color="#00AEEF" />}
                label="MÃ LỊCH HẸN"
              ...
              {appointment.note && (
                <View className="mt-3">
                  <View className="flex-row items-center mb-1">
                    <MaterialIcons name="notes" size={22} color="#00AEEF" className="mr-2" />
                    <Text className="text-[#8B949E] text-xs font-medium">GHI CHÚ</Text>
                  </View>
          ...
          >
            <View className="flex-row items-center mb-3">
              <MaterialIcons name="info-outline" size={22} color="#00AEEF" />
            ...
            <View className="pl-1">
              <View className="flex-row items-start mb-2">
                ...
                <Text className="text-[#8B949E] flex-1">
